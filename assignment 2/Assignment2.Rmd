...
```{r Backward Elimination AIC BIC, echo=TRUE}
library(MASS)
library(leaps)
...
```{r Forward Selection AIC BIC, echo=TRUE}
# Forward Selection
...
Now we apply Ridge regression to predict the target variable and utilize two methods: cross-validation and bootstrapping to determine the optimal regularization parameter $\lambda$. We are doing this because Ridge regression is particularly useful in reducing model complexity and handling multicollinearity by penalizing large coefficients and could improve our current situation. We explore a grid of $\lambda$ values and contrast the outcomes of the two methods, each aimed at finding the best balance between bias and variance.
...
predictions_test_knn <- knn(train = trainData[,-ncol(trainData)], 
                        test = testData[,-ncol(testData)],
                        cl = trainData$diabetes, 
                        k = k_value)

# Predictions on training set
predictions_train_knn <- knn(train = trainData[,-ncol(trainData)], 
                         test = trainData[,-ncol(trainData)],
                         cl = trainData$diabetes, 
                         k = k_value)

# Confusion matrix and accuracy for test set
confusion_matrix_test <- table(testData$diabetes, predictions_test_knn)
accuracy_test <- sum(diag(confusion_matrix_test)) / sum(confusion_matrix_test)
test_error_knn <- 1 - accuracy_test

# Confusion matrix and accuracy for training set
confusion_matrix_train <- table(trainData$diabetes, predictions_train_knn)
accuracy_train <- sum(diag(confusion_matrix_train)) / sum(confusion_matrix_train)
train_error_knn <- 1 - accuracy_train

# Print results
cat("Confusion Matrix (Test):\n")
print(confusion_matrix_test)
cat("Test Error Rate:", test_error_knn, "\n")

cat("Confusion Matrix (Train):\n")
print(confusion_matrix_train)
cat("Train Error Rate:", train_error_knn, "\n")
```
...
# Make predictions on the test set
pred_probs_test <- predict(gam_model2, newdata = testData, type = "response")
pred_class_test <- ifelse(pred_probs_test > 0.5, "pos", "neg") # Threshold at 0.5

# Confusion matrix and test error
conf_matrix_test <- confusionMatrix(as.factor(pred_class_test), testData$diabetes, positive = "pos")
accuracy_test <- conf_matrix_test$overall["Accuracy"]
test_error_GAM <- 1 - accuracy_test

# Make predictions on the training set
pred_probs_train_GAM <- predict(gam_model2, newdata = trainData, type = "response")
pred_class_train_GAM <- ifelse(pred_probs_train_GAM > 0.5, "pos", "neg")

# Confusion matrix and train error
conf_matrix_train <- confusionMatrix(as.factor(pred_class_train_GAM), trainData$diabetes, positive = "pos")
accuracy_train <- conf_matrix_train$overall["Accuracy"]
train_error_GAM <- 1 - accuracy_train

# Print results
cat("Test Error Rate:", test_error_GAM, "\n")
cat("Train Error Rate:", train_error_GAM, "\n")
```
...
```{r Ensemble of Bagged Tree, echo=TRUE}
set.seed(2024)
...
model_comparison <- data.frame(
  ...
  Training_Error = c(train_error_knn, train_error_GAM,
                     tree_train_error, 
                     bagged_train_error, rf_train_error, train_error),
  Test_Error = c(test_error_knn, test_error_GAM,
                 tree_test_error, bagged_test_error, rf_test_error, 
                 test_error)
)
...
ggplot(model_comparison_long, aes(x = Model, y = MSE, fill = Error_Type)) +
  ...
  labs(title = "Comparison of Training and Test Errors across Models",
       ...
       y = "Mean Squared Error (MSE)",
       fill = "Error Type") +
  scale_fill_manual(values = c("#4E79A7", "#F28E2B")) + # Custom colors
  theme_light() +
  theme(
    legend.position = "bottom",
    panel.grid = element_blank(),
